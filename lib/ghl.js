...
export async function sendAnalysisResults({
  ...
  if (sendEmail && contactData.email) {
    try {
      ...
      var emailResult = await sendEmailToConversation(ghlApiKey, conversationId, contactId, subject, html, contactData.email);
...
async function getOrCreateConversation(apiKey, locationId, contactId) {
  ...
  if (searchRes.ok) {
    ...
    if (searchRes.status === 401) {
      throw new Error("Invalid GHL API key — check your credentials in Settings.");
    }
    if (searchRes.status === 403) {
      throw new Error("GHL API key missing Conversations scope — enable it in GHL → Settings → Integrations.");
    }
  }
  ...
  if (!createRes.ok) {
    ...
    var cleanErr = createErrText.includes("scope") || createRes.status === 403
      ? "GHL API key is missing Conversations scope. Go to GHL → Settings → Integrations → Private Integrations → edit your token → enable Conversations (read + write)."
      : "GHL error " + createRes.status + ": " + createErrText.substring(0, 200);
...
async function sendEmailToConversation(apiKey, conversationId, contactId, subject, htmlBody, contactEmail) {
  var res = await fetch(GHL_BASE + "/conversations/messages", {
    method: "POST",
    headers: {
      "Authorization": "Bearer " + apiKey,

  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.id) {
        ...
        alert("Analysis complete! Score: " + (data.analysis?.score || "?") + "/10. DB error: " + (data.dbError || "unknown"));
        setLoading(false);
      }
    } catch (err) {
      setError(err.message);
          ...
          <div className="animate-fade-up">
            ...
            {step === 2 && (
              <div className="space-y-3">
                ...
                <Field label="TransUnion" type="number" value={form.scoreTU} onChange={v => update("scoreTU", v)} placeholder="720" />
                <Field label="Experian" type="number" value={form.scoreEX} onChange={v => update("scoreEX", v)} placeholder="715" />
                <Field label="Equifax" type="number" value={form.scoreEQ} onChange={v => update("scoreEQ", v)} placeholder="710" />
              </div>
            )}

            {step === 3 && (
              <div className="space-y-3">
                <Field label="Highest utilization on any card (%)" type="number" value={form.utilization} onChange={v => update("utilization", v)} placeholder="25" />
                <Field label="Number of open primary accounts" type="number" value={form.primaryAccounts} onChange={v => update("primaryAccounts", v)} placeholder="5" />
                <Field label="Average credit age (years)" type="number" value={form.creditAge} onChange={v => update("creditAge", v)} placeholder="3" />
                <Field label="Highest card limit ($)" type="number" value={form.highestLimit} onChange={v => update("highestLimit", v)} placeholder="15000" />
                <Field label="Hard inquiries per bureau (last 24 mo)" type="number" value={form.inquiries} onChange={v => update("inquiries", v)} placeholder="2" />
              </div>
            )}
            ...
            {step === 4 && (
              <div className="space-y-3">
                ...
                <Field label="Reporting errors found" type="number" value={form.errors} onChange={v => update("errors", v)} placeholder="0" />
                <Field label="Late payments in last 24 months" type="number" value={form.latePayments} onChange={v => update("latePayments", v)} placeholder="0" />
                <Field label="Negative items (collections, charge-offs, etc.)" type="number" value={form.negativeItems} onChange={v => update("negativeItems", v)} placeholder="0" />
              </div>
            )}

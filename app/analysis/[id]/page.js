...
export default function AnalysisDetail() {
  ...
  const scoreColor = pct >= 80 ? "var(--brand)" : pct >= 50 ? "var(--warning)" : "var(--danger)";
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        {/* Score Ring + Bureau Scores */}
        <div className="rounded-2xl p-6 mb-4 animate-fade-up" style={{ background: "var(--surface)", border: "1px solid var(--border)", animationDelay: "0.1s" }}>
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  const c = score >= 700 ? "var(--brand)" : score >= 650 ? "var(--warning)" : "var(--danger)";
                  return (
        ...
        {/* AI Summary */}
        {analysis.summary && (
          <div className="rounded-2xl p-5 mb-4 animate-fade-up" style={{ background: "rgba(57,255,20,0.04)", border: "1px solid rgba(57,255,20,0.15)", animationDelay: "0.2s" }}>
        ...
        {analysis.priority_actions?.length > 0 && (
          <div className="rounded-2xl p-5 mb-4 animate-fade-up" style={{ background: "var(--surface)", border: "1px solid var(--border)", animationDelay: "0.25s" }}>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-3 text-xs" style={{ color: "var(--text-muted)" }}>
              <span><span style={{ color: "var(--brand)" }}>●</span> {passed} passed</span>
              <span><span style={{ color: "var(--danger)" }}>●</span> {failed} needs work</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {results.map((r, idx) => {
              ...
              const color = r.passed ? "var(--brand)" : "var(--danger)";
              return (
                <div key={idx} className="rounded-xl overflow-hidden transition-all"
                  ...
                  {/* Header row */}
                  <button onClick={() => setExpandedCriteria(isExpanded ? null : idx)}
                    className="w-full px-4 py-3 flex items-center gap-3 text-left">
                    <span className="text-xs font-bold w-6 text-center" style={{ color }}>{String(r.criteriaId).padStart(2, "0")}</span>
                    <span className="text-sm flex-1 font-medium">
                      {["700+ FICO 8", "Personal Info", "Clean Report", "Utilization <30%", "4+ Primary Accts", "Credit Age 2+", "No Late Payments", "No Negatives", "$10K+ Card", "Max 2 Inquiries"][idx]}
                    </span>
                    <span className="text-xs font-medium px-2 py-0.5 rounded-md"
                      style={{ background: r.passed ? "rgba(15,175,60,0.12)" : "rgba(255,68,68,0.12)", color }}>
                      {r.passed ? "PASS" : "FAIL"}
                    </span>
                    <span className="text-xs" style={{ color: "var(--text-dim)" }}>{r.value}</span>
                    <svg className={`w-4 h-4 transition-transform ${isExpanded ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: "var(--text-dim)" }}>
